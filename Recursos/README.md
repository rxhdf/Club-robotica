# Recursos Generales

Esta carpeta contiene recursos adicionales, herramientas útiles y enlaces de referencia para complementar tu aprendizaje.

## 🛠️ Herramientas de desarrollo

### IDEs y Editores
- **Visual Studio Code**: Editor ligero y extensible
- **PyCharm**: IDE específico para Python
- **Visual Studio**: IDE completo para C++ y otros lenguajes
- **Jupyter Notebook**: Para experimentación interactiva
- **Google Colab**: Notebooks en la nube con GPU gratis

### Control de versiones
- **Git**: Sistema de control de versiones
- **GitHub**: Plataforma de alojamiento de código
- **GitLab**: Alternativa a GitHub
- **GitHub Desktop**: Cliente GUI para Git

### Línea de comandos
- **Terminal/CMD**: Interfaz de comandos
- **PowerShell**: Shell de Windows
- **Bash**: Shell de Linux/Mac
- **Oh My Zsh**: Framework para Zsh

## 📚 Plataformas de aprendizaje

### Cursos online
- [Coursera](https://www.coursera.org/)
- [edX](https://www.edx.org/)
- [Udacity](https://www.udacity.com/)
- [Udemy](https://www.udemy.com/)
- [Platzi](https://platzi.com/)
- [MIT OpenCourseWare](https://ocw.mit.edu/)

### Práctica de código
- [LeetCode](https://leetcode.com/)
- [HackerRank](https://www.hackerrank.com/)
- [Codewars](https://www.codewars.com/)
- [Exercism](https://exercism.org/)
- [Project Euler](https://projecteuler.net/)

### Competencias
- [Kaggle](https://www.kaggle.com/) - Data Science y ML
- [Codeforces](https://codeforces.com/) - Programación competitiva
- [TopCoder](https://www.topcoder.com/)
- [AtCoder](https://atcoder.jp/)

## 📖 Documentación oficial

### Lenguajes
- [Python Docs](https://docs.python.org/3/)
- [C++ Reference](https://en.cppreference.com/)
- [MDN Web Docs](https://developer.mozilla.org/)

### Frameworks y librerías
- [NumPy](https://numpy.org/doc/)
- [Pandas](https://pandas.pydata.org/docs/)
- [TensorFlow](https://www.tensorflow.org/)
- [PyTorch](https://pytorch.org/docs/)
- [Scikit-learn](https://scikit-learn.org/)

## 🎥 Canales de YouTube recomendados

### General
- [freeCodeCamp.org](https://www.youtube.com/c/Freecodecamp)
- [CS Dojo](https://www.youtube.com/c/CSDojo)
- [Traversy Media](https://www.youtube.com/c/TraversyMedia)

### Python
- [Corey Schafer](https://www.youtube.com/c/Coreyms)
- [Programming with Mosh](https://www.youtube.com/c/programmingwithmosh)
- [Tech With Tim](https://www.youtube.com/c/TechWithTim)

### IA y Machine Learning
- [Sentdex](https://www.youtube.com/c/sentdex)
- [Two Minute Papers](https://www.youtube.com/c/KárolyZsolnai)
- [StatQuest](https://www.youtube.com/c/joshstarmer)

### Matemáticas
- [3Blue1Brown](https://www.youtube.com/c/3blue1brown)
- [Khan Academy](https://www.youtube.com/c/khanacademy)
- [Numberphile](https://www.youtube.com/c/numberphile)

### C++
- [The Cherno](https://www.youtube.com/c/TheChernoProject)
- [CppCon](https://www.youtube.com/c/CppCon)

## 📝 Blogs y artículos

### Tecnología y programación
- [Medium - Towards Data Science](https://towardsdatascience.com/)
- [Dev.to](https://dev.to/)
- [Real Python](https://realpython.com/)
- [Analytics Vidhya](https://www.analyticsvidhya.com/)

### Papers académicos
- [arXiv.org](https://arxiv.org/) - Papers de investigación
- [Google Scholar](https://scholar.google.com/)
- [Papers With Code](https://paperswithcode.com/)

## 🌐 Comunidades

### Foros
- [Stack Overflow](https://stackoverflow.com/)
- [Reddit - r/learnprogramming](https://www.reddit.com/r/learnprogramming/)
- [Reddit - r/MachineLearning](https://www.reddit.com/r/MachineLearning/)
- [Reddit - r/cpp](https://www.reddit.com/r/cpp/)

### Discord y Slack
- Python Discord
- C++ Slack
- ML/AI communities

## 🔧 Herramientas útiles

### Diagramas y diseño
- [Draw.io](https://app.diagrams.net/)
- [Excalidraw](https://excalidraw.com/)
- [Figma](https://www.figma.com/)

### Colaboración
- [Google Colab](https://colab.research.google.com/)
- [Notion](https://www.notion.so/)
- [Trello](https://trello.com/)

### Regex y testing
- [Regex101](https://regex101.com/)
- [RegExr](https://regexr.com/)

### APIs y datos
- [Postman](https://www.postman.com/)
- [Public APIs](https://github.com/public-apis/public-apis)

## 📚 Libros recomendados

### Programación general
- "Clean Code" por Robert C. Martin
- "The Pragmatic Programmer" por Hunt & Thomas
- "Code Complete" por Steve McConnell

### Python
- "Python Crash Course" por Eric Matthes
- "Automate the Boring Stuff with Python" por Al Sweigart
- "Fluent Python" por Luciano Ramalho

### C++
- "C++ Primer" por Lippman, Lajoie, Moo
- "Effective Modern C++" por Scott Meyers

### IA y Machine Learning
- "Hands-On Machine Learning" por Aurélien Géron
- "Deep Learning" por Ian Goodfellow
- "Pattern Recognition and Machine Learning" por Christopher Bishop

### Matemáticas
- "Mathematics for Machine Learning" por Deisenroth, Faisal, Ong
- "Linear Algebra Done Right" por Sheldon Axler

## 🎓 Certificaciones

- **Python**: PCEP, PCAP (Python Institute)
- **Machine Learning**: Coursera ML Specializations
- **Cloud**: AWS, Google Cloud, Azure certifications
- **Data Science**: IBM Data Science Professional Certificate

## 💡 Consejos generales

1. **Aprende haciendo**: La mejor forma de aprender es practicando
2. **Proyectos personales**: Construye cosas que te interesen
3. **Lee código de otros**: Aprende de proyectos open source
4. **Participa en la comunidad**: Comparte conocimiento y ayuda a otros
5. **Mantente actualizado**: La tecnología cambia rápidamente
6. **No te rindas**: La programación es difícil, pero vale la pena
7. **Documenta tu aprendizaje**: Escribe sobre lo que aprendes

## 🔗 Enlaces útiles

### Gestores de paquetes
- [PyPI](https://pypi.org/) - Python Package Index
- [npm](https://www.npmjs.com/) - Node Package Manager
- [Anaconda](https://www.anaconda.com/) - Data Science Platform

### Datasets
- [Kaggle Datasets](https://www.kaggle.com/datasets)
- [UCI ML Repository](https://archive.ics.uci.edu/ml/)
- [Google Dataset Search](https://datasetsearch.research.google.com/)
- [Data.gov](https://data.gov/)

### Cheat Sheets
- [Python Cheat Sheet](https://www.pythoncheatsheet.org/)
- [NumPy Cheat Sheet](https://numpy.org/devdocs/user/quickstart.html)
- [Git Cheat Sheet](https://education.github.com/git-cheat-sheet-education.pdf)

## 🚀 Roadmaps

- [Developer Roadmaps](https://roadmap.sh/)
- [Data Science Roadmap](https://github.com/MrMimic/data-scientist-roadmap)
- [AI Expert Roadmap](https://github.com/AMAI-GmbH/AI-Expert-Roadmap)
